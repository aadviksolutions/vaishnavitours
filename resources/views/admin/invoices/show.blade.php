...
@section('page_title', 'Tax Invoice #' . $invoice->invoice_number)

@section('content')
<div class="d-flex align-center justify-between flex-wrap gap-2 mb-4 no-print">
    <div>
        <a href="{{ route('admin.invoices.index') }}" style="font-size: 0.875rem; color: var(--slate-600); font-weight: 600;">
            ← Back to All Invoices
        </a>
        <h1 style="font-size: 1.65rem; font-weight: 800; margin: 4px 0 0 0;">Tax Invoice & Travel Receipt</h1>
    </div>

    <div class="d-flex align-center gap-2">
        <a href="{{ route('admin.bookings.show', $invoice->booking_id) }}" class="btn btn-outline btn-sm">
            🚖 View Booking
        </a>
...
<div class="card invoice-sheet" style="background: #fff; max-width: 840px; margin: 0 auto; padding: 2.5rem; border: 1px solid var(--slate-200); box-shadow: var(--shadow-lg);">
    ...
    <div class="d-flex justify-between align-start mb-4" style="border-bottom: 2px solid var(--dark-900); padding-bottom: 1.5rem;">
        ...
        <div style="text-align: right;">
            ...
            <div style="font-size: 0.825rem; color: var(--slate-500);">
                <strong>Booking Ref:</strong> #{{ $invoice->booking->booking_id ?? $invoice->booking_id }}
            </div>
            <div class="mt-2">
    ...
    <div class="grid grid-2 gap-4 mb-4" style="font-size: 0.875rem;">
        ...
        <div style="background: var(--slate-50); padding: 1rem; border-radius: var(--radius-md);">
            ...
            <div><strong>Trip Type:</strong> {{ $invoice->booking->trip_type ?? 'N/A' }}</div>
            <div><strong>Pickup Date:</strong> {{ ($invoice->booking && $invoice->booking->travel_date) ? $invoice->booking->travel_date->format('d M Y') : '' }} at {{ $invoice->booking ? date('h:i A', strtotime($invoice->booking->travel_time)) : '' }}</div>
            ...
            @endif
            @if($invoice->booking && $invoice->booking->driver)
